...
function ResultMessage({ status, result }: { status: Status; result: VerificationResult | null }) {
  if (status === "success" && result) {
    return (
      <p>
        ...
        {result.name && <span> &mdash; Nombre: {result.name}</span>}
      </p>
    );
  }
  ...
  if (status === "network_error") {
    if (result?.error === "rate_limit") {
      return <p>Demasiadas solicitudes. Intenta nuevamente en un minuto.</p>;
    }
    if (result?.error === "token_not_configured") {
      return <p>Token de Meta no configurado. Contacta al equipo de desarrollo.</p>;
    }
